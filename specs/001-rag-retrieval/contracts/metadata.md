# ~~Contract: GET /metadata~~ — Renamed

> Endpoint renamed to `GET /stats`. See **[stats.md](stats.md)**.
> `/metadata` reads like an internal framework endpoint; `/stats` communicates intent to any caller.
...
No authentication required. Returns a detailed snapshot of what is currently deployed and indexed. Distinct from `/health` — health answers "is the service alive?", metadata answers "what exactly is running?"
