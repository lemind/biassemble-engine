...
- [x] T007 [US1] ⚠️ cross-repo (biassemble-core) **Verified done, retroactively checked off 2026-07-20** — never marked at the time. `biassemble-core`'s `src/rag/engine-client.ts` (`RagEngineClient`) calls `POST /retrieve-biases`, wired into `assessment.service.ts`, with an `AbortController`-driven timeout and `rag_fallback` logging on any error/timeout — the fallback mechanism this task asked for is real and present. **Deviation from the literal spec**: the timeout is `RAG_TIMEOUT_MS` (env-configurable, default `8000ms` in `src/lib/env.ts`, currently `240000ms` in `biassemble-core`'s own `.env`), not a hardcoded `500ms` — correctly adjusted since llm_union's real latency (~3-4s) wouldn't fit in 500ms; 500ms only survives today in `vitest.config.ts`'s test fixture.
...
- [x] T021 [US3] **Verified done, retroactively checked off 2026-07-20** — never marked at the time. `POST /evaluate` exists (`src/api/routes/retrieve.py:239`, auth-gated), `scripts/run_evaluation.py` calls it via `httpx` in remote mode, saves to `evaluations/runs/run_YYYY-MM-DD.json`, handles `--promote`; endpoint is in the README table. **Deviation from the literal spec**: the design evolved from a `latest`-cache read (`GET /evaluate/latest`) to an async job-queue pattern (`POST /evaluate` returns a `job_id`, `GET /evaluate/{job_id}` polls status/result) — a better fit once evaluation runs grew long enough to need progress polling and Space-restart recovery (spec-004/005's llm_union work). `specs/001-rag-retrieval/contracts/evaluate.md` was never written — a real, still-open doc gap, but the endpoint itself is real, live, and exercised constantly (this is how every remote eval this session ran).
