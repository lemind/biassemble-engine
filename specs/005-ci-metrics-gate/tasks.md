...
- [ ] T003 [P] Implement `scripts/check_regression.py` per [contracts/check-regression-cli.md](./contracts/check-regression-cli.md): `--run <path> --baseline <path>` args; for `negative` compute **one** `RegressionFinding` (`metric=empty_rate`); for `positive`/`edge`/`adversarial` compute **two independent** `RegressionFinding`s each (`metric=recall_at_k` and `metric=precision_at_k`, gated separately — seven findings total across today's four groups, not four) per [data-model.md](./data-model.md). For every finding: `tolerance = 1/count`; `eligible = tolerance < baseline_value` **computed per `(group, metric)` pair, not per group** (`edge`'s `recall_at_k` and `precision_at_k` have different baseline values and can land on opposite sides of eligibility — see data-model.md's worked note); `regressed = eligible and delta < -tolerance` (strict, one comparison direction for every finding — no per-group sign flip, per the corrected ADR §4 formula). Print the table shown in the contract (every `(group, metric)` pair, always, including ineligible/passing ones). Exit `0` (no eligible regression), `1` (eligible regression found), or `2` (missing/unreadable/malformed input file — distinct from `1` per the contract, so callers can tell "gate caught a regression" from "gate itself is broken").
- [ ] T004 [P] `tests/test_check_regression.py`: unit tests for the logic in T003, no eval infrastructure needed (pure functions over hand-built `GroupMetrics`/baseline dicts). Cover: (a) normal case — one-scenario-equivalent loss passes, two-scenario-equivalent loss fails (the `positive`, N=4 worked example in ADR §4); (b) `negative` group gates on `empty_rate` with the *same* `delta < -tolerance` direction as recall/precision, not an inverted one; (c) `adversarial`-shaped input (N=2, baseline 0.333) never sets `regressed=True` on either metric regardless of how far it drops, and this is *computed* from `count`/`baseline_value`, not a hardcoded group-name exception — assert this by constructing a synthetic group with the same `1/count ≥ baseline_value` shape under a different name and confirming it's also ineligible; (d) missing/malformed `--run` or `--baseline` file → exit `2`, not `1` or a traceback; (e) **per-metric eligibility split** — a group whose `recall_at_k` baseline is above `tolerance` but whose `precision_at_k` baseline is below it (the real `edge` shape: recall `0.833` eligible, precision `0.400` ineligible, same `tolerance=0.5`) produces two findings with *different* `eligible` values, proving eligibility is computed per `(group, metric)` pair and not inherited from the group as a whole.
