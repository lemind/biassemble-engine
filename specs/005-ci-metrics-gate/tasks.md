...
- [x] T009 [US3] Created `.github/workflows/production-drift.yml`: `schedule` (Monday 06:00 UTC) + `workflow_dispatch`, **no `pull_request` trigger at all** (verified via YAML parse — FR-010 satisfied structurally). **Bug caught before it could hit production**: `GET /evaluate/{job_id}` returns the raw `EvalRun` directly on completion (`return job["result"]` in `retrieve.py`), not a `{"status":"done",...}` wrapper — my first draft polled for `status=="done"`, which would have polled forever until timeout, every single week. Fixed to detect completion by the presence of `group_metrics` in the response instead, and to distinguish HTTP `500` (job failed) from other non-200s, both mapping to exit `2` ("could not evaluate"), separate from exit `1` ("evaluated, found a regression") — matches FR-012.
- [x] T010 [US3] **Validated live, not synthetically** — same `.env`/HF-token discovery as T008. Called the actual deployed Space: `POST /evaluate` → real `job_id`, polled every 10s exactly like the workflow does (13/13 scenarios), fetched the completed real `EvalRun`, ran `check_regression.py` against the new baseline — exit `0`, `positive` recall `0.729→0.562` (a real, known `llm_union` run-to-run swing, `HISTORY.md`'s documented range) stayed inside its `0.25` tolerance. Confirms the full Tier 2b mechanism end-to-end: dispatch, poll, completion-detection, comparison. Not validated: GitHub's own `schedule`/`workflow_dispatch` trigger actually firing this YAML file (same gap as T006 — no push to a real repo happened).
...
- [x] T012 Done by the user directly in GitHub's repo settings (not by any automation in this feature, per its own design) — verified via `gh api repos/lemind/biassemble-engine/branches/main/protection`: `required_status_checks.contexts = ["eval", "pytest"]`, `production-drift`/`drift-check` correctly absent (stays non-blocking per FR-010). PR #12 was merged and `production-drift.yml`'s first real dispatch on `main` passed before this was turned on, satisfying quickstart.md's own ordering ("only after Steps 1–5 are all green").
