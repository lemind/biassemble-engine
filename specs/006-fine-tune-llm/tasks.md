...
- [x] T020 [US3] Executed on Kaggle (T4, free-tier), documented in full in `candidate-2026-07-18-results.md`. Trained 3/3 epochs, `best_checkpoint_epoch=3` (`best_eval_loss=0.1778`; epoch 2's val loss ticked up to 0.312 then epoch 3 recovered — noise, not real overfitting). `target_modules` resolved via a regex scoped to the `language_model` subtree only (Gemma-3-4b-it is multimodal; an unscoped substring match would've also attached LoRA to the vision tower's identically-named `q_proj`/`k_proj`/`v_proj`) — 238 modules matched, 29.8M/4.33B trainable (0.69%), verified non-empty and non-attention-only. Smoke test: 20 random training-sample generations, 19/20 exact/near-exact match, 20/20 syntactically valid JSON with real catalog ids. Packaging (merge→GGUF) hit repeated real failures on this run (bitsandbytes tensor format, disk pressure, llama.cpp/tokenizer version skew — see T027) and was worked around for T022's trial eval via an adapter-only path; the actual merged+quantized GGUF was produced later under T027/T028.
- [x] T021 [US3] Written: `training/manifests/gemma3-4b-lora-2026-07-18.json`. Dataset version re-verified against the live file (`compute_dataset_version()` re-run against `evaluations/sft/sft_dataset.jsonl` reproduces `sha256:73ddf4ca...` exactly — dataset unchanged since T017's freeze), composition/per-bias counts pulled from `evaluations/sft/coverage_report.json`. LoRA hyperparameters and `base_model_revision` sourced from `candidate-2026-07-18-results.md`'s real run record (rank 16, alpha 32, dropout 0.05, lr 2e-4, 3/3 epochs, best_checkpoint_epoch 3, best_eval_loss 0.1778, resolved `target_modules` = the 7 language-model-only projections, 238 modules/29.8M params matched). `quantization_command` copied verbatim from `lora_finetune.md` §6. **One field genuinely unrecoverable**: `validation_split_fraction` was never recorded during the actual Kaggle run before that session died (T027) — recorded as `null` with an explanatory note rather than fabricated, per this file's own reproducibility rule (contracts/finetune-manifest-schema.md rule 2: a hand-typed/guessed value defeats the point). `eval_result` populated per T023 below.
...
- [x] T022 [US4] Ran (2026-07-18): `LLM_LORA_REPO`/`LLM_LORA_FILE` env override (production base GGUF + the trial LoRA adapter, per `src/llm/generator.py`'s optional path) → `evaluations/runs/run_2026-07-18.json` (93 scenarios, full `scenario_results` per-scenario array already present) → `check_regression.py --baseline baseline_2026-07-17.json` (results in `candidate-2026-07-18-results.md`). **Archiving backfilled 2026-07-19**: `training/manifests/gemma3-4b-lora-2026-07-18_predictions.json` written from the already-complete run — the same raw per-scenario data a fresh re-run would produce, since `run_2026-07-18.json` already had it; this task's archiving requirement was added to its own text after the run already happened, so nothing was actually re-executed to satisfy it.
- [x] T023 [US4] Ship gate verified against the real merged+quantized GGUF's run (`evaluations/runs/run_2026-07-19.json` vs `evaluations/baselines/baseline_2026-07-17.json`), by importing and calling `compute_findings()` directly (not parsing CLI output): `blind_spot` row **is present** (9 findings total across 5 groups, not silently omitted) — `recall_at_k` +0.1875 (0.325→0.5125, eligible, `regressed: false`) and `precision_at_k` +0.2177 (0.128→0.346, eligible, `regressed: false`), both pass. No other eligible group regressed past its own tolerance: `edge.recall_at_k` −0.167 (tolerance 0.5, not regressed), `positive.recall_at_k` −0.0625 (tolerance 0.25, not regressed), `positive.precision_at_k` +0.017 (improved). `blind_spot` precision improved alongside recall, so no recall-for-precision trade. Ship gate: **pass**. Full 9-entry `[dataclasses.asdict(f) for f in compute_findings(...)]` output written into T021's manifest `eval_result`.
...
- [x] T025 [US4] **Resolved — cost-free, and better than cost-free.** Compared `blind_spot`'s `scenario_results` between `run_2026-07-18.json` and `baseline_2026-07-17.json` directly (both already had the full per-scenario array). All 7 scenarios that flipped from a non-empty guess to `retrieved: []` are **negative-shaped `blind_spot` rows** (`expected: []`) where the baseline was hallucinating spurious labels (`narrative_fallacy`, `status_quo_bias`, `base_rate_neglect`, etc. — plausible-sounding but wrong) and scoring `recall_at_k: 0.0` on every one anyway, so baseline gained nothing from those guesses. Broader picture across all 20 negative-shaped `blind_spot` rows: baseline correctly returned empty on only **3/20**; the candidate correctly returns empty on **10/20** — more than triple, not a narrow 7-story fluke. Zero evidence found of the candidate going silent on a story that actually needed an answer — every empty-rate flip is confined to rows where empty is the *correct* answer. The `empty_rate` rise flagged as a caveat in ADR-006 §3/results-doc Priority 2 is a real precision improvement, not over-caution.
- [x] T026 [US4] **Skipped, per this task's own stated condition** — T025 found the flips cost nothing (baseline was already scoring 0 recall on all 7), so there's no real deficit to spend a retrain on. No hyperparameter pass or data-rebalancing performed.
- [x] T027 [US4] **Packaging pipeline fixed and executed for real — this is how T028's real merged+quantized GGUF got produced.** `training/lora_finetune.md` §6 and its "Session hygiene" section (added 2026-07-19, after a second attempt lost the trained adapter entirely to a session death) document every real trap hit and fixed: merge on **GPU** (`device_map="cuda"`, not `"cpu"` — the earlier CPU-merge advice is what caused the repeated OOMs; a free Kaggle T4's 16GB VRAM is a separate, mostly-empty pool once training's own objects are gone). `llama.cpp` built at the **exact** commit `llama-cpp-python==0.3.19` vendors (`c0159f9c1f874da15e94f371d136f5920b4b5335`), not `main`. Base model's **original**, unmodified tokenizer files copied directly into the merged checkpoint rather than letting `transformers.save_pretrained()` re-serialize them (avoids the BPE pre-tokenizer hash mismatch). Quantized via Q8_0 intermediate + `--allow-requantize` (direct Q4_K_M isn't supported by this converter version). Adapter uploaded to HF immediately after training, before the merge attempt. Result: `candidate.Q4_K_M.gguf` (2.49GB), `Leminds/gemma3-4b-bias-lora-candidate-2026-07-18`.
- [x] T028 [US4] **Re-evaluated the real merged+quantized GGUF — PASSES the gate, and slightly beat the proxy.** Real `candidate.Q4_K_M.gguf` (2.49GB, `Leminds/gemma3-4b-bias-lora-candidate-2026-07-18`) loaded through the true production path (`LLM_MODEL_REPO`/`LLM_GGUF_FILE`, no adapter trick) → `evaluations/runs/run_2026-07-19.json`. `check_regression.py` vs `baseline_2026-07-17.json`: exit 0. Per ADR-006 SC-004: `blind_spot` recall_at_k **+0.187** (0.325→0.512, eligible, pass) and precision_at_k **+0.218** (0.128→0.346, eligible, pass) — both strong eligible passes; no other eligible group regressed past tolerance (edge −0.167 and positive −0.062 both within their own noise tolerance); blind_spot precision went up so no recall-for-precision trade. Notably the real merged artifact edged out the adapter proxy (blind_spot recall 0.512 vs 0.481), confirming the quantized-merge didn't degrade the result. Per-scenario predictions archived at `training/manifests/gemma3-4b-lora-2026-07-18_merged-gguf_predictions.json`.
- [x] T029 [US4] **HACK audit DONE; production swap DONE and confirmed live.** HACK audit (`grep -rn "HACK(" src/`) surfaced exactly the two trial-eval LoRA-adapter tags in `src/config.py`/`src/llm/generator.py`; their `REVISIT` condition ("delete once T028's re-eval on the real merged GGUF supersedes this trial path") had fired, so the whole trial-eval `lora_repo`/`lora_file` path was deleted from `config.py`, `generator.py`, and `run_evaluation.py` — 151 tests still pass, no HACK tags remain. Config swap applied (`space-vars.env`'s `LLM_MODEL_REPO`/`LLM_GGUF_FILE` → the private candidate repo, synced, committed 6fff2e1). **Private-repo auth resolved 2026-07-19**: the Dockerfile's build-time GGUF prefetch was disabled (HF Spaces never expose Secrets — only Variables — during the Docker build; that `RUN` had no credential and 401'd on the now-private repo), and an `HF_TOKEN` Space secret was added so the runtime download in `generator.py` authenticates correctly. A separate, unrelated push blocker (a pre-LFS 13MB blob stuck in `hf` remote's would-be history) was worked around non-destructively — see `scripts/deploy_hf_space.sh`. **Confirmed live**: Space `stage: RUNNING`, `/health` → `llm_loaded:true`, `llm_model:"gemma3-4b-bias-lora-candidate-2026-07-18"`, `database_connected:true`. Only remaining follow-up (not part of this task): confirm `production-drift.yml`'s next run shows no regression.
...
- [x] T030 Final consolidated check, run for real against current repo state (2026-07-19): `pytest tests/ -v` → **151 passed** (including T004's 5 + T018's 13 new tests). Every planned `scripts/*.py` exists except `generate_sft_stories.py`, which is T012's documented, deliberate deviation (human-in-the-loop batch-prompt process instead), not a gap. Walked quickstart.md Steps 0-8 against real artifacts, not just checkboxes: Step 1 → `evaluations/sft/weak_supervision_pairs.jsonl` (28 rows). Step 2 → 80 `evaluations/blind_spot/*.json` files, `blind_spot` present in `baseline_2026-07-17.json`'s `group_metrics`. Step 5 → `evaluations/sft/sft_dataset.jsonl` (907 rows), `coverage_report.json` `pass: true`. Step 6 → `training/manifests/gemma3-4b-lora-2026-07-18.json` (T021). Step 7 → re-ran `check_regression.py --run run_2026-07-19.json --baseline baseline_2026-07-17.json` directly, exit 0, output numbers match T023/T028 exactly. Step 8 → `space-vars.env`'s `LLM_MODEL_REPO`/`LLM_GGUF_FILE` point at the shipped candidate, confirmed live via the Space's `/health` endpoint.
- [x] T031 **Post-deploy verification surfaced three real, previously-undetected bugs (2026-07-19/20), all fixed and confirmed live.** (1) The Dockerfile's `COPY evaluations/<group>` lines never included `blind_spot` — the deployed image structurally couldn't score it via `/evaluate` regardless of baseline state; fixed (`Dockerfile`), redeployed, confirmed live: `POST /evaluate?groups=blind_spot` on the running Space now returns `count: 80` with real metrics (recall 0.556, precision 0.356 — consistent with, not identical to, T028's local numbers; expected CPU-inference run-to-run variance). (2) `scripts/run_evaluation.py`'s remote-mode `_EVAL_GROUPS` constant was hardcoded to the original four groups and never updated — independent of the Dockerfile bug, this CLI would never have asked the server for `blind_spot` even with a fixed image; fixed. (3) `scripts/deploy_hf_space.sh` deploys `HEAD`'s **committed** tree, not the working directory — the first attempt at fixing (1) was edited but not yet committed when the script ran, so it silently deployed the *old* Dockerfile with no error; the script now refuses to run with uncommitted changes present. `production-drift.yml` still can't see `blind_spot` until `006-fine-tune-llm` merges to `main` (separate, still-open gap — the baseline on `main` predates `blind_spot`'s promotion, not a code bug).
