...
- [ ] T003 [P] Implement `scripts/validate_bias_catalog.py` — reuses `src/llm/prompt.py`'s `load_catalog(pool, taxonomy_version)` query path (DB-sourced, per that function's own docstring: "not hardcoded, not a re-parse of knowledge/*.md") to build the live `valid_ids: set[str]`, then exposes a function that rejects (does not coerce) any `bias_ids` entry outside that set. **`load_catalog` is `async` and takes a live `asyncpg.Pool`, not a connection string** — this script needs its own thin sync entrypoint: `asyncio.run(...)` wrapping `asyncpg.create_pool(settings.database_url)` (via `src/config.py`'s existing `Settings`) then calling `load_catalog(pool, settings.taxonomy_version)` — `settings.taxonomy_version` already exists (`src/config.py`, default `"2026-06-28"`) and is exactly what `src/api/app.py` already passes to this same function; do not invent a separate source or hardcode a taxonomy version string. Single source of truth imported by both T013 (labeling) and T016 (assembly) — per data-model.md's `BiasCatalogSnapshot` decision, this script must not hardcode a static 38-id list, since the catalog can drift.
- [ ] T004 [P] `tests/test_validate_bias_catalog.py` — deterministic tests against a fixture `valid_ids` set (no live DB call in the test itself): valid ids pass, an out-of-catalog id is rejected outright (not coerced, not silently dropped from a multi-label row while keeping the rest), human-readable-name-instead-of-id is rejected (e.g. `"Anchoring Bias"` vs `"anchoring_bias"`), empty `bias_ids` list is valid (negative examples).
- [ ] T005 [P] One-line change to `src/evaluation/evaluate.py`: add `"sft"` to `_SKIP_GROUPS`. **Verified necessary, not precautionary**: `load_scenarios` globs `group_dir.glob("*.json")` under every subdirectory of `evaluations/` not in `_SKIP_GROUPS` — a stray `.json` file placed in `evaluations/sft/` (a coverage report, a manifest copy) would otherwise be silently loaded as a malformed scenario and crash on missing `scenario_id`/`story` fields, the same class of bug the `"staging"` fix in spec-005 already hit once. Must land before T011 or T016 write anything to `evaluations/sft/`.
