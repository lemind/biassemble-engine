...
- [ ] T027 [US4] **Fix the packaging pipeline before producing the next candidate — follow `training/lora_finetune.md` exactly, don't repeat this session's saga.** That doc's §6 and its new "Session hygiene" section (added 2026-07-19, after a second attempt lost the trained adapter entirely to a session death) now cover every real trap hit so far: merge on **GPU** (`device_map="cuda"`, not `"cpu"` — the earlier CPU-merge advice is what caused the repeated OOMs, a free Kaggle T4's 16GB VRAM is a separate, mostly-empty pool once training's own objects are gone, not a "need different hardware" problem). Build `llama.cpp` at the **exact** commit `llama-cpp-python==0.3.19` vendors (`c0159f9c1f874da15e94f371d136f5920b4b5335`), not `main`. Copy the base model's **original**, unmodified tokenizer files directly into the merged checkpoint rather than letting `transformers.save_pretrained()` re-serialize them. Quantize via Q8_0 intermediate + `--allow-requantize` (direct Q4_K_M isn't supported by this converter version). **Upload the adapter to HF the moment training finishes, before attempting the merge** — the second attempt at this task had to fully retrain from scratch solely because the adapter was never backed up before the session that produced it died.
...
- [~] T029 [US4] **HACK audit DONE; production swap PENDING a human go-ahead + a deploy decision.** HACK audit (`grep -rn "HACK(" src/`) surfaced exactly the two trial-eval LoRA-adapter tags in `src/config.py`/`src/llm/generator.py`; their `REVISIT` condition ("delete once T028's re-eval on the real merged GGUF supersedes this trial path") had fired, so the whole trial-eval `lora_repo`/`lora_file` path was deleted from `config.py`, `generator.py`, and `run_evaluation.py` — 151 tests still pass, no HACK tags remain. **The actual production swap is deliberately NOT done automatically** — it's a production-facing change and the user's call. Two things must be decided/handled by whoever ships: (1) the candidate GGUF currently lives in a **private** HF repo (`Leminds/gemma3-4b-bias-lora-candidate-2026-07-18`), whereas the current production model (`bartowski/google_gemma-3-4b-it-GGUF`) is public — the HF Space must either be given auth to that private repo or the GGUF moved to a repo the Space can read; (2) the one-line `LLM_MODEL_REPO`/`LLM_GGUF_FILE` swap (`config.py` defaults or `space-vars.env`, ADR-003 §10's swap contract) + redeploy, then confirm via the already-live `production-drift.yml` that the next run shows no regression. The gate is cleared (T028); everything past this point is a deploy decision, not a quality question.
