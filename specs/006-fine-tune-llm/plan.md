...
Close the gap between `llm_union`'s current `positive` Recall@5 (0.729, currently promoted baseline) and its original target (0.85, ADR-003 §7 SC-001) by producing a LoRA-fine-tuned candidate GGUF and shipping it only if it clears the existing, already-built regression gate (specs/005-ci-metrics-gate). **Ship-gate update (2026-07-18, `adr/006-blind-spot-ship-gate.md`)**: real trial data showed the N=4 `positive` group too small to trust as a pass/fail signal on its own; the primary bar is now `blind_spot` (N=80) showing non-regressed recall and precision, with `positive` retained as a reported canary — see spec.md SC-004 and ADR-006 for the full reasoning. This is a data-and-training pipeline feature, not a service-code feature: it adds no new modules to `src/`, reuses `scripts/check_regression.py`/`retrieval-gate.yml` unchanged, and reuses the existing model-swap mechanism (`config.py`'s `llm_model_repo`/`llm_gguf_file`) unchanged **for the actual production swap**. Two small, necessary exceptions: `src/evaluation/evaluate.py`'s `_SKIP_GROUPS` gets one line added (`"sft"`) so the new training-data directory is never mistaken for a scenario group, and `src/llm/generator.py`/`config.py` gained an *optional*, default-off `llm_lora_repo`/`llm_lora_file` pair (`Llama(..., lora_path=...)`) used only for trial-evaluating a LoRA adapter over the production base GGUF before a proper merged-and-quantized candidate exists — production behavior is byte-for-byte unchanged when these are left unset. See Project Structure below. What's new is (1) a held-out evaluation group promoted *before* any training data exists, (2) an SFT dataset assembled from 28 reconstructed real weak-supervision pairs plus ≥300 synthetic stories, both validated against the 38-id bias catalog and spot-checked, and (3) a LoRA training/quantization/manifest pipeline producing a candidate in the exact runtime format already deployed.
