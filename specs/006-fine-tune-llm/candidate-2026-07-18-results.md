...
- **Resolved 2026-07-19 (T025), and it's good news, not a concern**: `blind_spot` `empty_rate` rose from ~3% to 12% — investigated by comparing raw `scenario_results` between the run and baseline directly. All flipped-to-empty scenarios are negative-shaped `blind_spot` rows (`expected: []`) where the baseline was hallucinating plausible-but-wrong labels and scoring 0 recall anyway. Across all 20 negative-shaped `blind_spot` rows, baseline correctly returned empty on only 3/20; the candidate correctly returns empty on 10/20. Zero evidence of the candidate going silent on a story that needed a real answer — this is a genuine precision improvement, not over-caution. See `tasks.md` T025 for the full comparison.
...
**Priority 2 — RESOLVED (T025, 2026-07-19), no longer a priority.** The `blind_spot` empty_rate rise (3%→12%) was investigated and found to be entirely composed of the model correctly learning to stay silent on negative-shaped stories (baseline correctly silent on 3/20, candidate on 10/20) — a real precision win, not over-caution. No rebalancing needed on this basis.
