...
- **SC-004** *(amended by `adr/006-blind-spot-ship-gate.md`, 2026-07-18 — see that ADR for the full rationale)*: A fine-tuned candidate's held-out `blind_spot` group (N=80) must show both recall_at_k and precision_at_k as eligible and non-regressed against the promoted baseline, per the existing regression-comparison tooling's unmodified tolerance rule; candidates that regress either are not shipped, and this determination requires no subjective judgment call. The original N=4 `positive`-group Recall@5 ≥ 0.85 bar is retained as a reported canary metric (still shown in every eval run) but no longer independently blocks shipping — N=4 was found too small to distinguish a real regression from single-story noise (ADR-006 §1).
- **SC-005**: SC-004 already requires `blind_spot`'s precision_at_k to be non-regressed, so a candidate cannot clear SC-004 by trading `blind_spot` recall for `blind_spot` precision — that trade-off is caught there, not restated here. This criterion covers the remaining case: a candidate is not promoted if it regresses `positive`, `adversarial`, or `edge` past their own eligibility/tolerance rule (these three do not have an independent precision requirement of their own; only their recall/tolerance regression is checked).
