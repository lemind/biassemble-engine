...
- **SC-004** *(amended by `adr/006-blind-spot-ship-gate.md`, 2026-07-18 — see that ADR for the full rationale)*: A fine-tuned candidate's held-out `blind_spot` group (N=80) must show both recall_at_k and precision_at_k as eligible and non-regressed against the promoted baseline, per the existing regression-comparison tooling's unmodified tolerance rule; candidates that regress either are not shipped, and this determination requires no subjective judgment call. **This requires the promoted baseline to actually contain a `blind_spot` entry** (FR-013's re-promotion prerequisite) — if it doesn't, `check_regression.py` produces zero findings for the group rather than an error, and SC-004 is not satisfiable at all in that state (not "passing," not "failing" — indeterminate). Treat a missing `blind_spot` row as a blocking prerequisite gap, never as an implicit pass. The original N=4 `positive`-group Recall@5 ≥ 0.85 bar is retained as a reported canary metric (still shown in every eval run) but no longer independently blocks shipping — N=4 was found too small to distinguish a real regression from single-story noise (ADR-006 §1).
- **SC-005**: SC-004 already requires `blind_spot`'s precision_at_k to be non-regressed, so a candidate cannot clear SC-004 by inflating `blind_spot` recall at precision's expense — e.g. naming more biases indiscriminately to catch more of the held-out set while also inventing more wrong ones — that specific trade-off (recall up, precision down) is caught there, not restated here. This criterion covers the remaining case: a candidate is not promoted if it regresses `positive`, `adversarial`, or `edge` past their own eligibility/tolerance rule (these three do not have an independent precision requirement of their own; only their recall/tolerance regression is checked).
...
- **Superseded 2026-07-18 by `adr/006-blind-spot-ship-gate.md`**: the original assumption here was that the pre-agreed numeric ship bar (Recall@5 ≥ 0.85 on the N=4 `positive` group) was inherited unchanged from the existing production quality target and this feature would not renegotiate that number. Real trial data (`candidate-2026-07-18-results.md`) showed that assumption was the actual blocker, not a settled given — ADR-006 replaces the primary bar with `blind_spot` (N=80, non-regression on recall and precision), keeping `positive` as a reported canary. See ADR-006 for the full evidence and reasoning; this line is left in place, struck through in spirit rather than deleted, so the original assumption and why it changed are both visible in one place.
