...
Same CLI as any PR (specs/005-ci-metrics-gate), unmodified — no new eval code path. **Confirm the table actually includes a `blind_spot` row before trusting its absence** — it only appears if the `--baseline` file was re-promoted after Step 2's promotion (research.md); a baseline that predates that re-promotion silently omits it rather than erroring, and treating a run as passing while `blind_spot` was never actually compared is not proven to generalize, it's untested. **As amended 2026-07-18 by `adr/006-blind-spot-ship-gate.md`**: confirm `blind_spot`'s `recall_at_k` **and** `precision_at_k` findings both read `eligible` and non-regressed (`"pass"`) — that pair is now the primary ship bar, not `positive`'s Recall@5 ≥ 0.85 (retained as a reported canary only, still shown in the table, no longer independently blocking). Also confirm no other group (`positive`/`adversarial`/`edge`) regressed past its own eligibility. When writing `eval_result` into the candidate's manifest, populate it with `[dataclasses.asdict(f) for f in compute_findings(...)]`, imported from `scripts/check_regression.py` directly (data-model.md) — it's a JSON array, not an object, and the raw dataclass list is not directly `json.dumps`-able.
