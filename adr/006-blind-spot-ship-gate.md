...
- **`blind_spot`'s `empty_rate` is not part of the new gate** (only `recall_at_k`/`precision_at_k` are) — worth flagging as a known gap, not silently accepted: the 2026-07-18 candidate's own trial run showed `blind_spot` `empty_rate` roughly tripling (0.0375 → 0.125 baseline-to-candidate). A candidate could clear this ADR's recall/precision bar while quietly getting more likely to return nothing at all on held-out stories. This is exactly the open question `candidate-2026-07-18-results.md`'s Priority 2 already flags for investigation — this ADR doesn't resolve it, and a future revision should consider whether `empty_rate` needs its own eligibility check once there's a basis for one.
